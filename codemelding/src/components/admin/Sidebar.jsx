...
const Sidebar = () => {
  ...
  return (
    <div
      className={`${sidebarWidth} bg-Orange px-5 sticky top-0 h-screen duration-300 overflow-hidden`}
    >
      ...
      <div className="flex gap-x-4 items-center">
        ...
        <h1
          className={`text-white origin-left font-semibold text-lg duration-200 ${scaleClass}`}
        >
        ...
        {Menus.map((menu, index) => (
          <li key={index}>
            <NavLink
              to={menu.link}
              className={({ isActive }) =>
                `flex rounded-md px-2 py-2.5 cursor-pointer text-White text-xs md:text-lg font-normal items-center gap-x-4 ${
                  isActive ? "bg-red" : "hover:text-red"
                }`
              }
            >
              {menu.icon}
